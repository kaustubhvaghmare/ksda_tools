...
def Fits2StarlightAscii(filename):
	"""
	Input: a FITS table spectrum
	Output: None.
	
	The input FITS file is converted into an ASCII form.
	"""
	fits_spectra = Table.read(filename)
	data = fits_spectra["FLUX"]
	wavelengths = fits_spectra["WAVELENGTH"]
...
def OldFits2StarlightAscii(filename):
	"""
